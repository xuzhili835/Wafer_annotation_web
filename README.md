...
- 12 类缺陷代码中文名已确认(2026-09-17,队友逐类看图+产线目录名交叉验证):
  X=隐裂、BX=崩边、HB=黑斑、HBB=黑斑-边部、HBX=黑斑-心部、XHB=线黑斑、HS=划伤、
  KD=孔洞、DQK=大缺角、XQK=小缺角、BYW=油污、KYW=颗粒油污。
  若数据方官方对照有出入,以官方为准,只改 `app/config.py` 的 `CODE_NAMES` 一张表。
